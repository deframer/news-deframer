...
// RenderItem renders a gofeed.Item to an RSS 2.0 XML string
func (f *feeds) RenderItem(ctx context.Context, item *gofeed.Item) (string, error) {
	rssItem := f.toRSS2Item(item)
	output, err := xml.MarshalIndent(rssItem, "", "  ")
	if err != nil {
...
// It ensures uniqueness of items based on their hash (first wins).
func (f *feeds) FilterItems(ctx context.Context, feed *gofeed.Feed, domains []string) []ItemHashPair {
	if feed == nil {
		return nil
	}

	var results []ItemHashPair
	...
	for _, item := range feed.Items {
		...
		results = append(results, ItemHashPair{Hash: key, Item: item})
	}
...
func SetExtension(item *gofeed.Item, prefix, name, value string) {
	if item.Extensions == nil {
		item.Extensions = make(ext.Extensions)
	}
	if item.Extensions[prefix] == nil {
		item.Extensions[prefix] = make(map[string][]ext.Extension)
	}
	item.Extensions[prefix][name] = []ext.Extension{{Name: name, Value: value}}
}
