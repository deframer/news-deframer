...
import { DomainComparison, DomainEntry, NewsDeframerApi, TrendMetric } from '@frontend-shared/ndf-api-interfaces';
...
export const TrendCompare = ({ baseItems, compareDomain, availableDomains, onSelectDomain, domain, searchEngineUrl, days, api }: TrendCompareProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const data = await api.getDomainComparison(domain.domain, compareDomain, domain.language, days);
        setItems(data);
      } catch (error) {
