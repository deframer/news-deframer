...
import { DomainEntry, NewsDeframerApi, TrendContext } from '@frontend-shared/ndf-api-interfaces';
...
export const TrendContextChart = ({ topic, className, days, domain, api }: TrendContextProps) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const data = await api.getContextByDomain(topic, domain.domain, domain.language, days || 7);
        if (mounted) {
          setItems(data);
        }
      } finally {
