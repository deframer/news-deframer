...
import { DomainEntry, NewsDeframerApi, TrendMetric } from '@frontend-shared/ndf-api-interfaces';
...
export const TrendTagCloud = ({ domain, days, searchEngineUrl, activeTab, setActiveTab, api }: TrendTagCloudProps) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const data = await api.getTopTrendByDomain(domain.domain, domain.language, days);
        setItems(data);
      } finally {
        setLoading(false);
      }
  ...
  return (
    <>
      ...
      {isSelectedTermVisible && selectedTerm && (
        <TrendDetails api={api} term={selectedTerm} domain={domain} days={days} activeTab={activeTab} setActiveTab={setActiveTab} />
      )}
    </>
  );
};
