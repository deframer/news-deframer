...
export const TrendDetails = ({ term, domain, days, showBorder = true, activeTab, setActiveTab, api }: TrendDetailsProps) => {
  ...
  return (
    <div className={`trend-details-container ${!showBorder ? 'no-border' : ''}`}>
      ...
      {activeTab === 'context' && <TrendContextChart api={api} topic={term} days={days} domain={domain} />}
      {activeTab === 'lifecycle' && <TrendLifecycleChart api={api} domain={domain} days={days} term={term} />}
      {activeTab === 'articles' && <ArticleList api={api} term={term} domain={domain} days={days} hideTitle={true} />}
    </div>
  );
};
