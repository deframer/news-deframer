...
export const Sentiments = ({ metricValues, tooltipId, onShowTooltip, onHideTooltip, className }: SentimentsProps) => {
  ...
  const renderMetrics = (metrics: SentimentMetric[]) => (
  ...
  const renderBe5Metrics = (metrics: SentimentMetric[]) => (
    <div className="sentiment-rows">
      {metrics.map((metric) => {
        const value = metricValues[metric.key as keyof typeof metricValues];
  ...
  return (
    <div className={`sentiment-viz ${className || ''}`}>
